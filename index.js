...
// 🎨 ANSI Color Codes for the Exact Reference Look
const blue = "\x1b[34m";
const cyan = "\x1b[36m";
const green = "\x1b[32m";
const reset = "\x1b[0m";
...
console.log(`${green}INFO [${time()}]: [0] Connecting...${reset}`);
console.log(`${green}INFO [${time()}]: [0] Installing Plugins...${reset}`);
...
for (const file of pluginFiles) {
    try {
        ...
        if (command.name) {
            ...
            // Matches the [0] Installed format from your reference
            console.log(`${green}INFO [${time()}]: [0] Installed ${command.name}${reset}`);
        }
    } catch (error) {
        console.log(`${green}INFO [${time()}]: [0] Failed to load ${file}${reset}`);
    }
}

console.log(`${green}INFO [${time()}]: [0] ${client.commands.size} External Plugins Installed${reset}`);
...
client.once('ready', async () => {
    ...
    try {
        ...
        console.log(`${blue}  ✉️  Owner DM notification failed.${reset}`);
    }
});
