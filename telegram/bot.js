...
async function sendLiveMessage(token, chatId, initialText, finalText, typingDelay = 1500) {
    ...
    const initialMsg = await sendTelegramMessage(token, chatId, initialText);
...
async function handleUpdate(client, token, update) {
    ...
    const context = { 
        ...
        reply: (text) => sendTelegramMessage(token, chatId, text),
        replyLive: (initial, final, delay) => sendLiveMessage(token, chatId, initial, final, delay)
    };
            ...
            if (['A', 'B', 'C', 'D'].includes(answer)) {
                ...
                if (isCorrect) {
                    await sendTelegramMessage(token, chatId, 
                        `✅ <b>CORRECT!</b> ${username}!\n\n` +
                        `The answer is <b>${game.question.a[game.correct]}</b>.\n` +
    ...
    if (text.startsWith('/')) {
        ...
        if (cmd) {
            try {
                ...
                await sendTelegramMessage(token, chatId, `❌ Error: ${e.message}`);
