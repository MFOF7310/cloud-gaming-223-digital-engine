...
// ================= MAIN SERVER LOGGING =================
const MAIN_LOG_CHANNEL_ID = process.env.MAIN_LOG_CHANNEL_ID; // Set in .env
...
async function logToMainServer(client, guild, action, data) {
    ...
    try {
        ...
        const logEmbed = new EmbedBuilder()
            .setColor('#2ecc71')
            .setAuthor({ name: '📋 GIVEAWAY LOG', iconURL: client.user.displayAvatarURL() })
...
async function creditWinners(client, db, winnersList, amount, guildName, guildId, lang) {
    ...
    for (const winnerId of winnersList) {
        try {
            // 🔥 FETCH CURRENT BALANCE FIRST (PER-SERVER: composite key)
            ...
            // Update database (PER-SERVER: includes guild_id)
...
// ================= INLINE BALANCE EMBED (for gw_check_balance button) =================
function buildBalanceEmbed(userData, lang, guildName, guildIcon, version) {
    ...
    return new EmbedBuilder()
        .setColor(credits >= 5000 ? '#2ecc71' : credits >= 1000 ? '#f1c40f' : '#e74c3c')
        .setAuthor({
...
async function announceInGiftChannel(client, db, guildId, winnersList, prize, hostName, amount, lang, version, guildName) {
    ...
    if (isMainServer) {
        ...
        // Use server_settings table instead of server_config
        try {
...
// 🛡️ ARCHITECT GUARD: Only intercept if an advanced ping flag was actively requested
if (mentionType !== 'none') {
    const botMember = message.guild.members.me;
    if (!botMember.permissions.has(PermissionsBitField.Flags.MentionEveryone)) {
        const permUpgradeEmbed = new EmbedBuilder()
            .setColor('#f39c12')
            .setAuthor({ name: '🦅 ARCHITECT ENGINE • SECURITY BLOCK', iconURL: client.user.displayAvatarURL() })
            .setTitle(lang === 'fr' ? '📢 Couche de Notification Avancée Verrouillée' : '📢 Advanced Notification Layer Locked')
            .setDescription(
                `\`\`\`ansi\n` +
                (lang === 'fr'
                    ? `\u001b[1;33m⚠️ SIGNATURE D'AUTORITÉ MANQUANTE\u001b[0m\n\nPour utiliser les pings globaux (\`--everyone\` / \`--here\`), ce nœud nécessite le niveau d'autorisation \u001b[1;32mMentionner Tout le Monde\u001b[0m.\n`
                    : `\u001b[1;33m⚠️ MISSING AUTHORITY SIGNATURE\u001b[0m\n\nTo utilize global pings (\`--everyone\` / \`--here\`), this node requires the \u001b[1;32mMention Everyone\u001b[0m clearance level.\n`) +
                `\`\`\`\n\n` +
                `> *${lang === 'fr' ? 'Cliquez sur le lien ci-dessous pour mettre à niveau les permissions.' : 'Click the secure link below to upgrade this bot instance\'s authorization.'}*`
            )
...
activeGiveaways.get(message.guild.id).set(giveawayId, giveaway);
        ...
        collector.on('collect', async (i) => {
            ...
            if (i.customId === 'gw_reroll') {
                ...
                const balanceMsg = await i.channel.send({
                    content: newWinners.map(w => `<@${w}>`).join(' '),
                    embeds: [winnerAnnouncement],
                    components: [balanceRow]
                }).catch(() => {});

                // 🔥 FIX: Collector for the balance check button on THIS message
                if (balanceMsg) {
                    ...
                    balanceCollector.on('collect', async (bi) => {
                        ...
                        // Fetch user's current balance (PER-SERVER)
                        const checkerId = bi.user.id;
                        let userData = null;
                        if (client.getUserData) {
                            userData = client.getUserData(checkerId, message.guild.id);
                        }
                        if (!userData) {
                            try {
                                userData = db.prepare("SELECT credits, level, xp FROM users WHERE id = ? AND guild_id = ?").get(checkerId, message.guild.id);
                            } catch (e) {}
                        }

                        const balanceEmbed = buildBalanceEmbed(userData, lang, guildName, guildIcon, version);
                        await bi.followUp({ embeds: [balanceEmbed], ephemeral: true }).catch(() => {});
                    });
                }
                
                if (currentGiveaway.amount > 0) {
                    await creditWinners(client, db, newWinners, currentGiveaway.amount, guildName, message.guild.id, lang);
                }
                
                await announceInGiftChannel(client, db, message.guild.id, newWinners, currentGiveaway.prize, currentGiveaway.hostName, currentGiveaway.amount, lang, version, guildName);
        ...
        setTimeout(async () => {
            ...
            try {
                ...
                if (winnersList.length > 0) {
                    ...
                    const balanceMsg = await message.channel.send({
                    ...
                    if (currentGiveaway.amount > 0) {
                        await creditWinners(client, db, winnersList, currentGiveaway.amount, guildName, message.guild.id, lang);
                    }
                    
                    await announceInGiftChannel(client, db, message.guild.id, winnersList, currentGiveaway.prize, currentGiveaway.hostName, currentGiveaway.amount, lang, version, guildName);
