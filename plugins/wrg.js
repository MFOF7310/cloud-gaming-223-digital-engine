...
data: new SlashCommandBuilder()
    .setName('wrg')
    .setDescription('🎮 Word guessing game with XP and credit rewards')
    .addStringOption(opt => opt
        .setName('difficulty')
        .setDescription('Difficulty level / Niveau de difficulté')
        .setRequired(false)
        .addChoices(
            { name: '🟢 Easy / Facile', value: 'easy' },
            { name: '🟡 Medium / Moyen', value: 'medium' },
            { name: '🟠 Hard / Difficile', value: 'hard' },
...
run: async (client, message, args, db, serverSettings, usedCommand) => {
        ...
        try {
            ...
            // 🔥 RAM-FIRST USER STATS (PER-SERVER)
            const guildId = message.guild?.id || 'DM';
            let userStats = client.getUserData
                ? client.getUserData(message.author.id, guildId)
                : db.prepare("SELECT xp, credits, level FROM users WHERE id = ? AND guild_id = ?").get(message.author.id, guildId);
            
            if (!userStats) {
                db.prepare("INSERT INTO users (id, guild_id, username, xp, level, credits) VALUES (?, ?, ?, 0, 1, 0)")
                    .run(message.author.id, guildId, message.author.username);
                userStats = { xp: 0, credits: 0, level: 1 };
                if (client.cacheUserData) client.cacheUserData(message.author.id, guildId, userStats);
            }
            ...
            collector.on('collect', async (m) => {
                ...
                if (guess === targetWord) {
                    ...
                    // 🔥 BATCH UPDATE FOR REWARDS (PER-SERVER)
                    const winnerGuildId = message.guild?.id || 'DM';
                    const winnerData = client.getUserData
                        ? client.getUserData(m.author.id, winnerGuildId)
                        : db.prepare("SELECT xp, credits, level, games_played, games_won FROM users WHERE id = ? AND guild_id = ?").get(m.author.id, winnerGuildId);
                    
                    if (!winnerData) {
                        db.prepare("INSERT INTO users (id, guild_id, username, xp, level, credits) VALUES (?, ?, ?, 0, 1, 0)")
                            .run(m.author.id, winnerGuildId, m.author.username);
                    }
                    ...
                    if (client.queueUserUpdate) {
                        client.queueUserUpdate(m.author.id, winnerGuildId, {
                            ...winnerData,
                            xp: newXP,
                            level: newLevel,
                        ...
                        db.prepare(`UPDATE users SET xp = xp + ?, credits = credits + ?, level = ?, games_played = COALESCE(games_played, 0) + 1, games_won = COALESCE(games_won, 0) + 1 WHERE id = ? AND guild_id = ?`)
                            .run(totalXP, totalCredits, newLevel, m.author.id, winnerGuildId);
                    }
...
if (message.guild) {
        ...
        if (duelistRoleId) {
            ...
            if (member) {
                ...
                if (role && !member.roles.cache.has(duelistRoleId)) {
                    await member.roles.add(role, '⚔️ Word guessing champion').catch(() => {});
                }
            }
        }
    } catch (e) {}
            ...
            const errorEmbed = new EmbedBuilder()
                ...
                .setFooter({ text: `ARCHITECT CG-223 • v${client.version || '1.8.0'}` })
                .setTimestamp();
    ...
    execute: async (interaction, client) => {
        ...
        const usedCommand = lang === 'fr' ? 'devine' : 'wrg';
