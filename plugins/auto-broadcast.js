...
function buildEmbed(changes, lang, guild, version) {
    ...
    if (!hasChanges) {
        ...
        if (changes.new.length > 0) {
            ...
            changes.new.forEach(c => { const label = c.isCore ? 'Core Engine' : humanName(c.file); lines.push(`• ${t.newPlugin(label, c.desc)}`); });
            lines.push('');
        }
        if (changes.fixed.length > 0) {
            lines.push(`**${t.fixedHeader}**`);
            changes.fixed.forEach(c => { const label = c.isCore ? 'Core Engine' : humanName(c.file); lines.push(`• ${t.fixedPlugin(label, c.desc)}`); });
            lines.push('');
        }
        if (changes.improved.length > 0) {
            lines.push(`**${t.improvedHeader}**`);
            changes.improved.forEach(c => { const label = c.isCore ? 'Core Engine' : humanName(c.file); lines.push(`• ${t.improvedPlugin(label, c.desc)}`); });
            lines.push('');
        }
        if (changes.updated.length > 0 && changes.new.length + changes.fixed.length + changes.improved.length === 0) {
            lines.push(`**${t.updatedHeader}**`);
            changes.updated.forEach(c => { const label = c.isCore ? 'Core Engine' : humanName(c.file); lines.push(`• ${t.updatedPlugin(label, c.desc)}`); });
            lines.push('');
        }
        if (changes.removed.length > 0) {
            lines.push(`**${t.removedHeader}**`);
            changes.removed.forEach(c => { lines.push(`• ${t.removedPlugin(humanName(c.file))}`); });
            lines.push('');
        }
    }
