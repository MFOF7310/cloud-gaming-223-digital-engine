/**
 * ARCHITECT CG-223 // NEURAL AUDIO ENGINE v4.0 LavalinK
 * Lavalink-based streaming - Hetzner-proof, cookie-free
 * Supports: YouTube, SoundCloud, Spotify, Apple Music, Bandcamp
 *
 * By: Moussa Fofana // Node BAMAKO_223
 * Stack: discord.js + shoukaku + Lavalink v3
 */
...
async function playTrack(guildId, track) {
  ...
  if (!s.player) {
    throw new Error('Not connected to voice. Join a voice channel first!');
  }
  ...
  try {
    ...
    s.textChannel?.send({
      embeds: [new EmbedBuilder().setColor(C.RED).setDescription('Failed to play **' + track.title + '**\n`' + err.message + '`')]
    }).catch(() => {});
    s.current = null;
...
async function connectVoice(guildId, voiceChannel, textChannel) {
  ...
  if (!node) throw new Error('Lavalink not connected. Check server.');
...
function buildNPEmbed(guildId) {
  ...
  if (!track) {
    return new EmbedBuilder()
      ...
      .setFooter({ text: 'ARCHITECT CG-223 // Neural Audio v4.0 Lavalink' })
      .setTimestamp();
  }
...
module.exports = {
    ...
    switch (cmd) {
      case 'play': case 'p': {
        ...
        try {
          ...
          if (!s.player || s.player.state !== 'CONNECTED') {
            await connectVoice(gid, vc, message.channel);
          }

          if (s.current) {
            s.queue.push(track);
            loading.edit({ embeds: [e(C.GREEN, 'Queued: [' + track.title + '](' + track.url + ')')] });
          } else {
          ...
          loading.edit({ embeds: [e(C.RED, err.message)] });
        }
        break;
      }
      case 'search': {
        await sendSearchMenu(message.channel, rest.join(' '), message.author, gid);
        break;
      }
      case 'skip': case 's': {
      ...
      case 'queue': case 'q': {
        if (!s.current) return message.reply({ embeds: [e(C.WARN, 'Queue empty!')] });
        const em = new EmbedBuilder().setColor(C.ACCENT).setTitle('Queue - ' + (s.queue.length + 1) + ' tracks');
        em.addFields({ name: 'Now', value: '[' + s.current.title + '](' + s.current.url + ')' });
        const list = s.queue.slice(0, 15).map((t, i) => '`' + (i + 1) + '.` [' + t.title + '](' + t.url + ')').join('\n');
      ...
      case 'volume': case 'vol': {
        ...
        if (isNaN(vol) || vol < 0 || vol > 200) return message.reply({ embeds: [e(C.WARN, '0-200')] });
        s.volume = vol;
        if (s.player) await s.player.setVolume(vol / 100);
    ...
    switch (action) {
      case 'play': {
        ...
        try {
          ...
          if (!s.player || s.player.state !== 'CONNECTED') {
            await connectVoice(gid, vc, interaction.channel);
          }

          if (s.current) {
            s.queue.push(track);
            await interaction.editReply({ embeds: [e(C.GREEN, 'Queued: [' + track.title + '](' + track.url + ')')] });
          } else {
            await playTrack(gid, track);
            await sendNowPlaying(gid, interaction.channel);
            await interaction.editReply({ embeds: [e(C.GREEN, 'Now Playing: [' + track.title + '](' + track.url + ')')] });
          }
        } catch (err) {
          await interaction.editReply({ embeds: [e(C.RED, err.message)] });
        }
        break;
      }
      case 'search': {
        await sendSearchMenu(interaction.channel, interaction.options.getString('query'), interaction.user, gid);
        await interaction.editReply({ embeds: [e(C.GREEN, 'Pick a track from the menu!')] });
        break;
      }
  ...
  // Lavalink init (call from index.js)
  initLavalink,
};

console.log('Neural Audio Engine v4.0 Lavalink loaded - Hetzner-proof streaming');
