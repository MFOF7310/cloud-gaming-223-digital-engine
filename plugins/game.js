...
const texts = {
    fr: {
        ...
        triviaNote: "🧠 **TRIVIA DISPONIBLE!** Utilisez `.trivia` pour jouer au quiz de culture générale!",
        triviaNotFound: "❌ Commande trivia non trouvée."
    },
    en: {
        ...
        triviaNote: "🧠 **TRIVIA AVAILABLE!** Use `.trivia` to play the general knowledge quiz!",
        triviaNotFound: "❌ Trivia command not found."
    }
};
...
function updateGameStats(userId, won, winnings, channel, username, lang, client) {
    ...
    checkAndAnnounceLevelUp(oldXp, newUser?.xp || 0, userId, username, channel, lang, client);
    
    return actualWinnings;
}

function hasEnoughCredits(userId, betAmount) {
...
module.exports = {
    ...
    run: async (client, message, args, userData, serverSettings, usedCommand) => {
        ...
        switch(subCommand) {
            case 'dice': case 'dé': return playDiceGame(client, message, userData, args[1], lang, serverSettings);
            case 'coinflip': case 'cf': case 'pileface': return playCoinFlip(client, message, userData, args[1], lang, serverSettings);
            case 'guess': case 'number': case 'devine': return playNumberGuess(client, message, userData, lang, serverSettings);
            case 'slots': case 'slot': case 'machine': return playSlots(client, message, userData, lang, serverSettings);
            case 'blackjack': case 'bj': case 'vingtetun': return playBlackjack(client, message, userData, lang, serverSettings);
            case 'rps': case 'rockpaperscissors': case 'pfc': return playRPS(client, message, userData, args[1], lang, serverSettings);
            case 'hangman': case 'hm': case 'pendu': return playHangman(client, message, userData, lang, serverSettings);
            case 'leaderboard': case 'lb': case 'classement': return showGameLeaderboard(client, message, args[1], lang, serverSettings);
            case 'stats': case 'statistiques': return showGameStats(client, message, userData, lang, serverSettings);
            case 'rank': case 'rang': case 'profil': return showAgentProfile(client, message, userData, lang, serverSettings);
            default:
                ...
                const errorEmbed = new EmbedBuilder()
                    ...
                    .setFooter({ text: `${guildName} • NEURAL ARCADE • v${client.version || '1.5.0'}`, iconURL: guildIcon });
...
async function showGameMenu(client, message, oldData, lang, serverSettings, usedCommand) {
    ...
    const menuEmbed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: '💡 ASTUCE', value: t.triviaNote, inline: false }
        )
    ...
    collector.on('collect', async (interaction) => {
        ...
        switch(interaction.customId) {
            case 'game_dice': await playDiceGame(client, message, userData, null, lang, serverSettings); break;
            case 'game_coinflip': await playCoinFlip(client, message, userData, null, lang, serverSettings); break;
            case 'game_slots': await playSlots(client, message, userData, lang, serverSettings); break;
            case 'game_blackjack': await playBlackjack(client, message, userData, lang, serverSettings); break;
...
// ================= DICE GAME =================
async function playDiceGame(client, message, userData, guess, lang, serverSettings) {
    const t = texts[lang];
    const bet = 100;
    const guildName = message.guild.name.toUpperCase();
    const guildIcon = message.guild.iconURL() || client.user.displayAvatarURL();
    
    if (!hasEnoughCredits(message.author.id, bet)) {
...
// ================= COIN FLIP =================
async function playCoinFlip(client, message, userData, choice, lang, serverSettings) {
    const t = texts[lang];
    const bet = 100;
    const guildName = message.guild.name.toUpperCase();
    const guildIcon = message.guild.iconURL() || client.user.displayAvatarURL();
...
async function playNumberGuess(client, message, userData, lang, serverSettings) {
    const t = texts[lang];
    const bet = 100;
    const guildName = message.guild.name.toUpperCase();
    const guildIcon = message.guild.iconURL() || client.user.displayAvatarURL();
    ...
    const embed = new EmbedBuilder()
        ...
        .setFooter({ text: `${guildName} • ${lang === 'fr' ? 'Vous avez 60 secondes!' : 'You have 60 seconds!'} • v${client.version || '1.5.0'}`, iconURL: guildIcon });
    ...
    collector.on('collect', async (msg) => {
        ...
        if (guess === game.target) {
            ...
            updateGameStats(message.author.id, true, totalGain, message.channel, message.author.username, lang, client);
            ...
            updateGameStats(message.author.id, false, 0, message.channel, message.author.username, lang, client);
            activeGuesses.delete(message.author.id);
            collector.stop();
...
// ================= SLOT MACHINE =================
async function playSlots(client, message, userData, lang, serverSettings) {
    const t = texts[lang];
    const bet = 100;
    const guildName = message.guild.name.toUpperCase();
    const guildIcon = message.guild.iconURL() || client.user.displayAvatarURL();
...
// ================= BLACKJACK =================
async function playBlackjack(client, message, userData, lang, serverSettings) {
    const t = texts[lang];
    const bet = 200;
    const guildName = message.guild.name.toUpperCase();
    const guildIcon = message.guild.iconURL() || client.user.displayAvatarURL();
...
// ================= ROCK PAPER SCISSORS =================
async function playRPS(client, message, userData, choice, lang, serverSettings) {
    const t = texts[lang];
    const bet = 100;
    const guildName = message.guild.name.toUpperCase();
    const guildIcon = message.guild.iconURL() || client.user.displayAvatarURL();
...
// ================= HANGMAN =================
async function playHangman(client, message, userData, lang, serverSettings) {
    const t = texts[lang];
    const bet = 200;
    const guildName = message.guild.name.toUpperCase();
    const guildIcon = message.guild.iconURL() || client.user.displayAvatarURL();
    ...
    collector.on('collect', async (m) => {
        ...
        if (getDisplay() === targetWord) {
            ...
            updateGameStats(message.author.id, true, winnings, message.channel, message.author.username, lang, client);
            collector.stop('win');
        } else if (lives <= 0) {
            updateGameStats(message.author.id, false, -bet, message.channel, message.author.username, lang, client);
            collector.stop('lose');
        } else {
...
// ================= GAME STATS =================
async function showGameStats(client, message, userData, lang, serverSettings) {
    const t = texts[lang];
    const version = client.version || '1.5.0';
    const guildName = message.guild.name.toUpperCase();
    const guildIcon = message.guild.iconURL() || client.user.displayAvatarURL();
...
// ================= AGENT PROFILE =================
async function showAgentProfile(client, message, userData, lang, serverSettings) {
    const t = texts[lang];
    const version = client.version || '1.5.0';
    const guildName = message.guild.name.toUpperCase();
    const guildIcon = message.guild.iconURL() || client.user.displayAvatarURL();
...
// ================= GAME LEADERBOARD =================
async function showGameLeaderboard(client, message, type, lang, serverSettings) {
    const t = texts[lang];
    const version = client.version || '1.5.0';
    const guildName = message.guild.name.toUpperCase();
    const guildIcon = message.guild.iconURL() || client.user.displayAvatarURL();
